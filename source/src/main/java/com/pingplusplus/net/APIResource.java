...
import com.bond.allianz.utils.BasicAuthenticator;
import com.bond.allianz.utils.PropertyConfig;
    ...
    private static java.net.HttpURLConnection createPingppConnection(
        ...
        if (pingppURL.getProtocol().equals("https")) {
            if(Boolean.parseBoolean(isProxy)) {//添加代理
                conn = (HttpsURLConnection) pingppURL.openConnection(getProxy());
            }else {
                conn = (HttpsURLConnection) pingppURL.openConnection();
            }
        } else {
            if(Boolean.parseBoolean(isProxy)) {//添加代理{
                conn = (HttpURLConnection) pingppURL.openConnection(getProxy());
            }else {
