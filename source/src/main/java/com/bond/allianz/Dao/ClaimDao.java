...
//vehicleVesselTax,vehiclePurchaseTax,vehicleLicenseFee,vehicleBZPremium,vehicleBusinessPremium,partsLossSum,hoursLossSum
        ...
        sql="select dealerno,status,vin,claimstatus,icode from  contract where contractno ='"+contractno+"'";
        ...
        if (domainenter.equals("1")&&claimtype.contains("车辆置换"))
        {
            icode="CCIC";
        }
        //多品牌的时候根据理赔类型来控制保险公司
        ...
        {
            if (claimtype.contains("轮胎"))icode="CHLF";
            else  if (claimtype.contains("车辆置换"))icode="CCIC";
            else icode="CPIC";
        }
      ...
      if (claimno.equals(""))
      {
          ...
          if (claimtype.equals("PS-车辆置换服务")||domainenter.equals("0"))
          {
              String Invoiceurl="",insuranceurl="";
              sql="select Invoiceurl,insuranceurl from contract where contractno=?";
              ...
              {
                  sql="insert into claimpic (tid,claimno,picurl,valid,userid,pictype) values ( uuid(),?,?,1,?,?)";
      ...
      int ii=  jdbcTemplate.update(sql,queryList);

        if (status.contains("提交"))
        {
            sql="update  contract  set claimstatus='理赔已提交' where  contractno =?";
            jdbcTemplate.update(sql,contractno);
            sql="update claimpic set candel=0 where claimno=?";
            ...
            sql="insert into claimcheck (status,checkuserid,ttime,checkremark,claimno) values  (?,?,"+tt+",?,?)";
...
////        }
    ...
    public  void  createPdf(String contractno) {
        ...
        {
            if (print_version==3)
                model = new File(root, "images/paicmodel3.pdf").getPath();
            else
        ...
        {
            if (print_version==3)
                model = new File(root, "images/cpicmodel3.pdf").getPath();
            else
       ...
       // if (!carstarttime.equals(""))startdate=carstarttime;
        if (map!=null) {
            ...
            if (disc.contains("代步保障")||disc.contains("出险保障")) {
               ...
               // Date submit_date=parseDate(map.get("submittime")+" 12:00:00","yyyy-MM-dd HH:mm:ss");
                ...
                {
                    if(  parseDate(carstarttime,"yyyy-MM-dd").after(parseDate(start3,"yyyy-MM-dd")))
                        start3 =carstarttime;
                }

                end3 = addYear(start3, setting.StrToInt(map.get("accident") + ""));
            }
        }
        mappdf.put("xh1",xh1);
        mappdf.put("xh2",xh2);
    ...
    private String getClaimno()
    {
        ...
        int t1=GetMonthtime();
        long  ii=0;
        String sqlstring="select max(claimno) as claimno from claim where ttime>"+t1;
    ...
    public String getClaim(String data){
        ...
        sql="select contract.brand,contract.vin,claim.*,FROM_UNIXTIME(claim.submittime,'%Y-%m-%d %H:%i:%s') as dsubmittime,FROM_UNIXTIME(claim.ttime,'%Y-%m-%d %H:%i:%s') as createtime,FROM_UNIXTIME(claim.checktime,'%Y-%m-%d %H:%i:%s') as claimtime,dealer.dealername,insurance.iname as insurancename,contract.cname from  claim LEFT OUTER JOIN dealer on dealer.dealerno=claim.dealerno inner join insurance on insurance.code=claim.icode inner join contract on contract.contractno=claim.contractno where 1=1 ";
        if (!contractno.equals(""))
        {
            sql+=" and claim.contractno like ?";
            queryList.add("%"+contractno+"%");
        }

        if (!keystring.equals("")) {
            sql += " and (claim.vin like  ?  or contract.cname  like ? or claim.linkman like ? or  claim.contractno like  ?)";
        ...
        if (!submituserid.equals("")){
            sql+=" and claim.submituserid=?";
            queryList.add(submituserid);
        }
        if (!claimno.equals("")){
            sql+=" and claim.claimno=?";
            queryList.add(claimno);
        }
        if (!claimpro.equals("")){
            sql+=" and claim.claimtype in ('"+claimpro.replace(",","','")+"')";
        }
        ...
        if (!status.equals("")){
            if (status.equals("未处理"))sql+=" and claim.status like '%已提交'";
            else if (status.equals("已处理"))sql+=" and (claim.status like '%已批准' or claim.status like '%已拒绝' or claim.status like '%已结案'  )";
            else if (status.equals("报告"))sql+=" and (claim.status !='草稿' and claim.status !='' and claim.status is not null)";
           else {
               sql+=" and claim.status=?";
                queryList.add(status);
            }

        }
        if (!claimtype.equals("")){
            sql+=" and claim.claimtype=?";
            queryList.add(claimtype);
        }
        if (!province.equals("")){
            sql+=" and claim.province=?";
        ...
        sql+=" order by claim.submittime desc ,claim.ttime desc ";
        String resultss="";
    ...
    public String getClaimCount(String data){
        ...
        String sql ="";
        sql="select status from  claim  where contractno=? and claimtype=? ";
    ...
    public String getClaimRemark(String data){
        ...
        String sql ="";
        ;
        sql="select claimcheck.tid,claimcheck.checkremark as remark,claimcheck.checkuserid,claimcheck.claimno,FROM_UNIXTIME(claimcheck.ttime,'%Y-%m-%d %H:%i:%s') as createtime,claimcheck.status,usermember.username,usermember.truename from  claimcheck LEFT OUTER JOIN usermember on usermember.id=claimcheck.checkuserid  where claimcheck.claimno=? order by ttime ";
    ...
    public String checkClaim(String data){
        ...
        sql="select contractno,icode,dealerno from claim where claimno=?";
        ...
        sql="update claim set status=?,checkuserid=?,checktime="+tt+",checkremark=? where claimno=?";
        args=new Object[4];
        args[0]=status;
        args[1]=checkuserid;
        args[2]=checkremark;
     ...
     {
         sql="update claim set payamount=?,paytime="+tt+"  where claimno=?";
         args=new Object[2];
         args[0]=payamount;
         args[1]=claimno;
         jdbcTemplate.update(sql,args);
     ...
     String resultss="";
        if (ii==1) {
            ...
            sql="select max(tid) as id from claimcheck";
    ...
    public String addClaimpic(String data, String filetype, MultipartFile file ){
        ...
        String sql ="";
        sql="insert into claimpic (tid,claimno,picurl,valid,userid,pictype,candel) values ( uuid(),?,?,1,?,?,1)";
        Object[] parms=new Object[4];
       parms[0]=claimno;
        parms[1]=picurl;
        parms[2]=userid;
        ...
        jdbcTemplate.update(sql,parms);
        String resultss="";
        sql="select tid,claimno,picurl,userid,pictype from claimpic where claimno=? and picurl=?";
        parms=new Object[2];
        parms[0]=claimno;
        parms[1]=picurl;
    ...
    public String getClaimpic(String data){
        ...
        sql="select tid,claimno,picurl,pictype,candel from claimpic where valid=1 and claimno=? order by createtime desc";
   ...
   String     sql="update claimpic set valid=0 where tid=?";
    ...
    public String claim_down(String data){
        ...
        sql="select (@i:=@i+1)   as   i,c.vin,c.brand,u.tel,c.Invoiceprice,c.Invoicedate,u.username as userid,FROM_UNIXTIME(cl.paytime,'%Y-%m-%d %H:%i:%s') as paytime,cl.*,FROM_UNIXTIME(cl.ttime,'%Y-%m-%d %H:%i:%s') as createtime,FROM_UNIXTIME(cl.submittime,'%Y-%m-%d %H:%i:%s') as submittime1,FROM_UNIXTIME(cl.checktime,'%Y-%m-%d %H:%i:%s') as claimtime,d.dealername,i.iname as insurancename,c.cname,c.dealerno as contract_dealerno \n" +
                ...
                "from  claim cl LEFT OUTER JOIN dealer d on d.dealerno=cl.dealerno inner join insurance i on i.code=cl.icode inner join contract c on c.contractno=cl.contractno left outer join usermember u on cl.submituserid=u.id ,(SELECT @i:=0) as i where 1=1 ";
        if (!contractno.equals(""))
        {
        ...
        {
            title.put("dealerstate","理赔经销商");
        }
       if (domainenter.equals("3"))
       {

           title.put("dangerdate","出险日期");
           title.put("vin","车架号");
           title.put("tel","经销店经办人联系电话");
           title.put("brand","品牌");
           title.put("Invoiceprice","原车发票价");
           title.put("Invoicedate","原车发票日期");
           title.put("vehicleVesselTax","重置新车车船税");
           title.put("vehiclePurchaseTax","重置新车购置税");
           title.put("vehicleLicenseFee","重置新车上牌费");
           title.put("vehicleBZPremium","重置新车交强保费");
           title.put("vehicleBusinessPremium","重置新车商业保费");
           title.put("partsLossSum","配件金额");
           title.put("hoursLossSum","工时金额");
       }
    ...
    public String addorupdateclaimtier(String data){
        ...
        JsonObject jsonObject = new JsonParser().parse(data).getAsJsonObject();
        String appid="",nonce="",time="",sign="",claimno="",brandname="",tiretype="",dot="",tid="";
     ...
     //   if (jsonObject.keySet().contains("data"))jsonObject=jsonObject.get("data").getAsJsonObject();
        ...
        for (int i=0;i<jsonArray.size();i++) {
            ...
            if (jsonObject.keySet().contains("claimno")) claimno = jsonObject.get("claimno").getAsString();
            if (jsonObject.keySet().contains("brandname")) brandname = jsonObject.get("brandname").getAsString();
            if (jsonObject.keySet().contains("tiretype")) tiretype = jsonObject.get("tiretype").getAsString();
            if (jsonObject.keySet().contains("dot")) dot = jsonObject.get("dot").getAsString();

            if (i==0)
            {
                sql="delete from claimtire where claimno=?";
                jdbcTemplate.update(sql, claimno);
            }
            tid="";
            ...
            if (tid.equals("")) {
                ...
                sql = "insert into claimtire (ttime,claimno,brandname,tiretype,dot) values (" + tt + ",?,?,?,?)";


            } else {
                sql = "update claimtire set claimno=?,brandname=?,tiretype=?,dot=? where tid=" + tid;
            }
            jdbcTemplate.update(sql, args);
            if (tid.equals(""))
            {
                sql="select tid from claimtire where ttime="+tt;
                Map<String, Object> map = queryForMap(jdbcTemplate, sql);
                if (map == null) tid="";
                else {
                    tid = map.get("tid") + "";
    ...
    public String getclaimtierdetail(String data){
        ...
        JsonObject jsonObject = new JsonParser().parse(data).getAsJsonObject();
        String appid="",nonce="",time="",sign="",claimno="";
        int multi=0;
        String result="";
        if (jsonObject.keySet().contains("appid"))appid=jsonObject.get("appid").getAsString();
        if (jsonObject.keySet().contains("nonce"))nonce=jsonObject.get("nonce").getAsString();
        ...
        String sql ="";
        int tt=setting.GetCurrenntTime();
        sql="select * from claimtire where claimno=?";
    ...
    public List<Map<String,Object>> getClaimPic(String claimnos) {
        ...
        String sql = "select * from claimpic where valid=1 and claimno in ('" + String.join("','", claimnos.split(",")) + "')";
    ...
    public List<Map<String,Object>> selectCheckPicList(int tid,String claimno ) {
        String sql = "select * from claimcheckpic where 1=1 ";
        List<Object> queryList = new ArrayList<Object>();
        if (tid > 0) {
            sql += " and tid = ?";
            queryList.add(tid);
    ...
    public int insertCheckPic(int tid,String claimno,String name,String picurl) {
        int flag=0;
        try {
            String sql = "insert into   claimcheckpic (tid,claimno,name,picurl) values(?,?,?,?) ";
            flag = jdbcTemplate.update(sql, new Object[]{tid, claimno, name, picurl});

            sql="select max(id) as id from claimcheckpic";
    ...
    public  int deleteCheckPicByID(int id){
        String sql ="delete from claimcheckpic where id=? ";
