...
(function () {
  ...
  /**
   * @params radian 为tsz修改后的独有参数    用来修改连线之间的弧度    范围区间为-1 ~ 1   数字越大越平直
   */
  function CurveLine(points, opts) {
    var self = this;
    var curvePoints = getCurvePoints(points, opts);
     ...
     * 重写弧线的编辑功能
     */
    polyline.enableEditing = function (opts) {
      var self = this;
      ...
      if (self.map) {
        ...
        for (var i = 0; i < self.cornerPoints.length; i++) {
          ...
          marker.addEventListener('dragend', function () {
            ...
            var curvePoints = getCurvePoints(self.cornerPoints, opts)
   ...
   * 根据弧线的坐标节点数组
   */
  function getCurvePoints(points, opts) {
    var curvePoints = [];
    for (var i = 0; i < points.length - 1; i++) {
      var p = getCurveByTwoPoints(points[i], points[i + 1], opts);
      if (p && p.length > 0) {
   ...
   * @param Point 终点
   */
  function getCurveByTwoPoints(obj1, obj2, opts) {
    ...
    if (t2 == 0) {
      // t2 = (t + (Math.PI / opts.radian ? (4.5 * opts.radian) + 3 : 5));
      t2 = (t + (Math.PI / 5 / (1 + opts.radian)));
    }
    h2 = h / 2 / (1 + opts.radian);
