...
function getQueryString(val) {
    var qs = val.split('?')[1], // 获取url中"?"符后的字串  
        args = {}, // 保存参数数据的对象
...
function setLayout() {
    ...
    //菜单没有任何变化
    $(".sidebar").height(screenHt - 78);
    siderbarHt = $(".sidebar").height();
    $(".mainFrame").height(siderbarHt - 78);
...
function updateGuideSecond(ts) {
    ...
    $(".guideList li:nth-of-type(3) a").attr("href", second_href).text(second_text);
...
function second_menuCur(c, cl) {
    ...
    for (var i = 0; i < second_menuList.length; i++) {
        if (i == c) {
            ...
            var dataId = second_menuList.eq(i).attr('data-idx');
            var valGQS=getQueryString(ifmSrc);
            if (valGQS.funcId) {
                if(!token){
                    if(WaitNum>10){
                        alert('dms验证未成功，请稍后重试')
                        $('.top-tabs').find('[key='+dataId+'] .u-close').trigger('click');
                        $('.second-menu').eq(0).trigger('click');
                        return false;
                    }
                    WaitNum=WaitNum+1;
                    setTimeout(function(){
            ...
            ifmSrc = ifmSrc + "&token=" + token;
            WaitNum=0;
            }else{
            var clientWidth = document.documentElement.clientWidth || document.body.clientWidth;
            var ifmSrc = ifmSrc + "&clientWidth=" + clientWidth;
            }
            var text = second_menuList.eq(i).text();
            //if(ifmSrc.indexOf(".") != -1){
            if (ifmSrc && $('.iframes iframe').length == 1 && !$('.iframes iframe').eq(0).attr('src')) {
                $(".mainFrame").eq(0).attr("src", ifmSrc); //记录当前iframe的src，刷新的时候保持停留在该页
                $(".iframes li").eq(0).attr("key", dataId);
                $(".top-tabs li").eq(0).attr("key", dataId);
                $(".top-tabs li").eq(0).html(text);
            } else {
                return false;
            }
